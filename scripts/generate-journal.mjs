...
# YORON BBQ（ヨロンバーベキュー）の思想 — 正本 bbq-site/AN-BBQ-PHILOSOPHY.md の凝縮
- ブランド名は「YORON BBQ（ヨロンバーベキュー）」＝与論島発のバーベキュー。本場アメリカンBBQを、与論島から。英字表記 YORON BBQ が正。旧称「あんBBQ／ニューバーベキュー」は使わない。
- アメリカンBBQの優れたロジック（蓋付きグリル・間接熱・ロー&スロー・温度の科学）は継承する。ただし12時間級の長時間燻製は日本の暮らしに根づかない。その限界を超えて日常に溶け込むのが「YORON BBQ」。
- コピーの3原則: ①5〜6時間で完結（作って食べて楽しんで全部込み。12時間は憧れとして語ってよいが推奨はこちら） ②熱々を、出来立てで（冷めた塊肉＋ソースにしない。冷めて乾いたプルドポークにBBQソースをかけるのは、A5焼肉を自家製タレで食べるのと同じロジック） ③多品種（肉2〜3種＋副菜・シーフード。普通のスーパーの食材で）。
...
- A系統=アメリカンBBQ探求の入口記事: ロー&スロー・ラブ・温度管理・ブリスケットなど、本場アメリカンBBQに憧れて検索してきた読者向け。憧れと探求心にまっすぐ応える。
  ただし結びで一段だけ、自然にYORON BBQへの橋を架ける——読者が遠からず突き当たる同じ課題（12時間は現実的でない・塊肉は冷める・単調）にそっと触れ、「5〜6時間で完結する与論島発のやり方（YORON BBQ）もある」と示す。橋は1段だけ。説教や宗旨替えの強要にせず、アメリカンBBQ探求そのものは否定しない。
